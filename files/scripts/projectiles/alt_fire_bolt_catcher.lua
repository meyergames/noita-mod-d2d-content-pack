...
local actionid = "action_d2d_alt_fire_bolt_catcher"
...
if GameGetFrameNum() >= cooldown_frame then
    if is_alt_fire_pressed() then
        ...
        if ( mana > manacost and ( uses_remaining ~= 0 or is_always_cast ) ) then
            ...
            GameShootProjectile( root, x+aim_x*12, y+aim_y*12, x+aim_x*20, y+aim_y*20, EntityLoad( "mods/D2DContentPack/files/entities/projectiles/deck/bolt_catcher.xml", x, y ) )
            wand.mana = mana - manacost

            local spells, attached_spells = wand:GetSpells()
            for i,spell in ipairs( spells ) do
                if ( spell.action_id == "D2D_BOLT_CATCHER_ALT_FIRE" ) then
                    ...
                    if ( uses_remaining == 1 ) then
                        ...
                        EntityLoad("mods/D2DContentPack/files/particles/fade_alt_fire_bolt_catcher.xml", x, y )
                    end
            ...
            if HasFlagPersistent(actionid) == false then
                ...
                if ModIsEnabled( "quant.ew" ) then
                    CrossCall( "d2d_ew_alt_fire", root, x, y, aim_x, aim_y, "mods/D2DContentPack/files/entities/projectiles/deck/bolt_catcher.xml" )
                end
