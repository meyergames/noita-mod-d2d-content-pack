...
local pull_radius = get_internal_int( entity_id, "pull_radius" )
if pull_radius == nil then pull_radius = 0 end 
...
function calculate_force_at(body_x, body_y)
	...
	local gravity_percent = ( pull_radius - distance ) / pull_radius
...
for _,id in ipairs(entities) do	
	...
	if physicscomp == nil and not EntityHasTag(id,"pk_flash") then
