...
local actionid = "action_d2d_blink_mid_fire_t3"
    ...
    if InputIsMouseButtonDown( 3 ) then -- is the middle mouse button pressed?
        ...
        if ( mana >= manacost and ( uses_remaining ~= 0 or is_always_cast ) ) then
            ...
            for i,spell in ipairs( spells ) do
                if ( spell.action_id == "D2D_BLINK_MID_FIRE_T3" ) then
            ...
            EntityInflictDamage( root, math.min( p_max_hp * 0.02, p_hp - 0.04 ), "DAMAGE_SLICE", "experimental teleportation", "NONE", 0, 0, root, x, y, 0)
