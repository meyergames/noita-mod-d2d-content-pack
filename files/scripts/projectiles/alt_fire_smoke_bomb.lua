    ...
    if isButtonDown_AltFire() then
        ...
        if ( mana > manacost and ( uses_remaining ~= 0 or is_always_cast ) ) then
            ...
            for i,spell in ipairs( spells ) do
                if ( spell.action_id == "CTQ_SMOKE_BOMB_ALT_FIRE" ) then
            ...
            if HasFlagPersistent(actionid) == false then
                ...
                if ModIsEnabled("quant.ew") then
                    CrossCall("ctq_ew_alt_fire", root, x, y, aim_x, aim_y, "mods/RiskRewardBundle/files/entities/projectiles/deck/smoke_bomb.xml" )
                end
